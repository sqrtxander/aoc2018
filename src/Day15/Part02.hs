module Day15.Part02 (solution) where
...
tick :: (Monad m) => Set Coord -> (Maybe Unit -> m (Maybe Unit)) -> Map Coord Unit -> m (Map Coord Unit, Bool)
tick walls attack = go M.empty
  where
    go past current
        | M.null current = return (past, False)
        | otherwise = do
            ...
            if M.null enemies
                then
                    return (M.insert pos unit otherUnits, True)
                else
                    if M.null orthogonalEnemies
                        then
                            ...
                            (target, _) : _ -> do
                                past' <- M.alterF attack target (M.insert pos' unit past)
                                future' <- M.alterF attack target future
                                go past' future'

outcome :: World -> Int
outcome World{wallsW = walls, unitsW = units} = head [result | power <- [4 ..], Just result <- [tryPower power]]
  where
    tryPower power = runIdentity $ simulate power
    simulate :: Int -> Identity (Maybe Int)
    simulate power = do
        runMaybeT $ do
            (finalUnits, rounds) <- go 0 units power
            return (rounds * sum (map hpU $ M.elems finalUnits))
    go :: Int -> Map Coord Unit -> Int -> MaybeT Identity (Map Coord Unit, Int)
    go rounds units' elfPower = do
        (units'', done) <- tick walls (attack elfPower) units'
        if done
            then return (units'', rounds)
            else go (rounds + 1) units'' elfPower
    attack :: Int -> Maybe Unit -> MaybeT Identity (Maybe Unit)
    attack power (Just Unit{unitTypeU = Goblin, hpU = hp})
        | hp > power = return . Just $ Unit{unitTypeU = Goblin, hpU = hp - power}
    attack _ (Just Unit{unitTypeU = Elf, hpU = hp})
        | hp > 3 = return . Just $ Unit{unitTypeU = Elf, hpU = hp - 3}
        | otherwise = MaybeT $ return Nothing
    attack _ _ = return Nothing
